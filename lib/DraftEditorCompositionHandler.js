...
function getComposedText(snapshot) {
  ...
  if (blockNode && typeof blockText === 'string') {
    ...
    var composedText = getCompositionTextFromDOM(blockText, endText, domSelection) || getCompositionTextFromDOM(blockText, endText);
...
var DraftEditorCompositionHandler = {
  ...
  onCompositionStart: function onCompositionStart(editor) {
    ...
    // composition. If the previous session is still actively composing (a
    // `compositionstart` fired again without an intervening
    // `compositionend`), `resolveComposition` would be a no-op, so keep the
    // original snapshot instead of overwriting it; the DOM diff then still
    // spans the whole composition.
    if (!stillComposing && compositionSnapshot != null && !resolved) {
  ...
  onCompositionEnd: function onCompositionEnd(editor, e) {
    resolved = false;
    stillComposing = false; // Some IMEs fire `compositionend` multiple times for the same session
    ...
    if (compositionSnapshot && compositionSnapshot.composedText == null && e && e.data) {
   ...
   */
  resolveComposition: function resolveComposition(editor, continueComposition) {
    if (stillComposing) {
      return;
    }
